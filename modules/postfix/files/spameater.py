...
def getReplyAddress(fromAddress, toAddress):
  ...
  # Do fromAddress already sent an email to toAddress?
  execQuery("SELECT `mailAddress` FROM `replyAddress` WHERE disposableMailAddress = '" + toAddress + "' AND foreignAddress = '" + fromAddress + "';")
...
def main():
  ...
  for line in originalMail:
    ...
    if r:
      ...
      finalMailReplyTo = getReplyAddress(r.group(1), recipient)
  ...
  if not finalMailFrom:
    logging.warning("Can not retrieve From information. Using sender (" + sender + ")")
    finalMailFrom = getReplyAddress(sender, recipient)
  if not messageId:
